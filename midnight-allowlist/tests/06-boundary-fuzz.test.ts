...
describe('Boundary & Fuzz Testing', () => {
  ...
  describe('hash function properties', () => {
    ...
    it('domain separation: hashLeaf("") !== zero hash at level 0', () => {
      // Zero nodes hash the all-zero 32-byte buffer (required for on-chain
      // sparse-tree validity); hashLeaf("") normalizes via SHA-256("") which
      // is a non-zero digest. Domain separation guarantees they diverge.
      const zeros = computeZeroHashes(1);
      const emptyLeaf = hashLeaf('');
      expect(emptyLeaf).not.toBe(zeros[0]);
    });
  });
